...
function WelcomeScreen({ route }: any) {
  ...
  const handleSubmit = async (values: User) => {
    if (values.nation) {
      const tempNation = nationList.find((item) => item.name === values.nation);
      values.nation = tempNation;
    }
    values.email = values.email?.toLowerCase();

    const result: ApiResponse<any> | any = await authApi.loginRegister(values);
        ...
        <ScrollView contentContainerStyle={styles.form}>
          ...
          >
            <FormGroupPicker
              ...
              name="SignInOrUp"
              list={signList}
              fixedPadding={Platform.OS === 'ios' ? 20 : 40}
              onChange={(name: string, value: string) => setSignMode(value)}
            />
            {signMode === 'SignUp' && (
              <FormField
                name="firstName"
            ...
            {signMode === 'SignUp' && nationSelectList.length > 0 && (
              <FormPicker
                name="nation"
            ...
            {signMode === 'SignIn' && (
              <TouchableOpacity
                onPress={() => navigation.navigate(routes.FORGOT_PASSWORD.name)}
              >
            ...
            <SubmitButton
              title={signMode === 'SignUp' ? 'Sign Up' : 'Sign In'}
              backgroundColor={signMode === 'SignUp' ? 'primary' : 'secondary'}
            />
          </Form>
        </ScrollView>
      </Screen>
